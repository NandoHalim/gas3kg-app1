...
export default function DashboardView({ stocks: stocksFromApp = {} }) {
  ...
  // 2) Hitung ringkasan keuangan yang benar
  useEffect(() => {
    let alive = true;
    const calculateFinancialSummary = async () => {
      try {
        // Gunakan DataService untuk mendapatkan data yang akurat
        const salesData = await DataService.getSalesHistory({
          from: '2000-01-01', // Semua data
          to: todayStr(),
          method: "ALL",
          status: "ALL",
          limit: 10000
        ...
        // Filter hanya transaksi yang dibayar (Tunai atau LUNAS) dan tidak dibatalkan
        const paidSales = (salesData || []).filter(sale => {
          const status = String(sale.status || '').toUpperCase();
          const method = String(sale.method || '').toUpperCase();
          return status !== 'DIBATALKAN' && 
                 (method === 'TUNAI' || status === 'LUNAS');
        });

        // Hitung omzet, HPP, dan laba
        const omzet = paidSales.reduce((sum, sale) => sum + (Number(sale.total) || 0), 0);
        const totalQty = paidSales.reduce((sum, sale) => sum + (Number(sale.qty) || 0), 0);
        const hpp = totalQty * hppSetting;
        ...
        const todaySales = await DataService.getSalesSummary({ 
          from: todayStr(), 
          to: todayStr() 
        });
        setToday(todaySales || { qty: 0, money: 0 });
...
function ComparisonCard({ title, current, previous, growth, type = "qty" }) {
  ...
  return (
    <Card variant="outlined" sx={{ p: 2, background: alpha(isPositive ? theme.palette.success.main : theme.palette.error.main, 0.05) }}>
