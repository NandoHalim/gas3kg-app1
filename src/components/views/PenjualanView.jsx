...
import { DEFAULT_PRICE, PRICE_OPTIONS, PAYMENT_METHODS, COLORS } from "../../utils/constants.js";
...
export default function PenjualanView({ stocks = {}, onSaved, onCancel }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      onSaved?.(snap);
      setForm({ customer: "", qty: "", price: DEFAULT_PRICE, method: "TUNAI" });
    } catch (e) {
      setErr(e.message);
    } finally {
      setLoading(false);
        ...
        <div className="grid">
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <Button onClick={() => inc(-1)} disabled={loading || qtyNum <= 1} title="Kurangi">
                −
              </Button>
              <Input
                type="number"
