...
export default function StokView({ stocks = {}, onSaved, onCancel }) {
  ...
  const handleAddIsi = async () => {
    ...
    try {
      ...
      onSaved?.(snap);
      setFormIsi({ qty: '', note: '' });
      toast?.show?.({ type: 'success', message: '✅ Stok ISI bertambah (mengurangi KOSONG)' });
    } catch (e) {
    ...
    <div className='grid' style={{ gap: 16 }}>
      ...
      <Card title="Tambah Stok ISI (butuh KOSONG)">
        <div className='grid' style={{ gap: 8 }}>
          ...
          <div>
            <Button onClick={handleAddIsi} disabled={loading}>
              {loading ? 'Processing...' : 'Tambah ISI'}
            </Button>
          </div>
        </div>
      </Card>
      ...
      <Card title="Tambah Stok KOSONG">
        <div className='grid' style={{ gap: 8 }}>
          ...
          <label>Catatan</label>
          <Input
            placeholder='contoh: beli tabung kosong'
            value={formKosong.note}
            onChange={e => setFormKosong({ ...formKosong, note: e.target.value })}
            disabled={loading}
          />
          <div>
            <Button onClick={handleAddKosong} disabled={loading}>
              {loading ? 'Processing...' : 'Tambah KOSONG'}
            </Button>
          </div>
        </div>
      </Card>
