...
import {
  Box, Stack, Typography, Card, CardHeader, CardContent, TextField,
  Button, Alert, Grid, FormControl, InputLabel, Select, MenuItem, Chip, Tooltip,
} from "@mui/material";
  ...
  return (
    <Stack spacing={2}>
      <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 1 }}>
        <Typography variant="h5" fontWeight={700}>Stok</Typography>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={4}><TambahKosong onSaved={(s) => { setSnap(s); onSaved?.(s); }} /></Grid>
        <Grid item xs={12} md={4}><RestokIsi onSaved={(s) => { setSnap(s); onSaved?.(s); }} /></Grid>
        <Grid item xs={12} md={4}><PenyesuaianStok onSaved={(s) => { setSnap(s); onSaved?.(s); }} /></Grid>
      </Grid>
    </Stack>
  );
}
...
function TambahKosong({ onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const snap = await DataService.addKosong({ qty: qtyNum, date: form.date, note: form.note });
      toast?.show?.({ type: "success", message: `Stok KOSONG +${qtyNum}` });
      setForm({ qty: "", date: todayStr(), note: "" }); onSaved?.(snap);
    } catch (e2) {
      const msg = e2.message || "Gagal tambah KOSONG"; setErr(msg); toast?.show?.({ type: "error", message: msg });
    ...
    <Card>
      ...
      <CardContent>
        ...
        <Box component="form" onSubmit={submit}>
          <Stack spacing={1.5}>
            <TextField {...FIELD_PROPS} sx={FIELD_SX}
              label="Jumlah" type="number" inputProps={{ min: 1 }}
            ...
            />
            <TextField {...FIELD_PROPS} sx={FIELD_SX}
              label="Catatan (opsional)" multiline minRows={2}
...
function RestokIsi({ onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const msg = e2.message || "Gagal restok ISI"; setErr(msg); toast?.show?.({ type: "error", message: msg });
    } finally { setLoading(false); }
  };
...
function PenyesuaianStok({ onSaved }) {
  ...
  const [form, setForm] = useState({
    code: "KOSONG", dir: "OUT", qty: "", date: todayStr(), reason: "",
  });
  const [loading, setLoading] = useState(false);
  const [err, setErr] = useState("");
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const msg = e2.message || "Gagal penyesuaian stok"; setErr(msg); toast?.show?.({ type: "error", message: msg });
    } finally { setLoading(false); }
  };
        ...
        <Box component="form" onSubmit={submit}>
          <Stack spacing={1.5}>
            <FormControl fullWidth>
              ...
              <Select labelId="jenis-label" label="Jenis Stok"
            ...
            <FormControl fullWidth>
              ...
              <Select labelId="arah-label" label="Arah Penyesuaian"
            ...
            />
            <TextField {...FIELD_PROPS} sx={FIELD_SX}
              label="Alasan (wajib)" multiline minRows={2}
